...
func test_violet_case_selects_three_independent_case_states() -> void:
	var game := _spawn(&"violet_case")
	assert_eq(game.save_state()["case_states"].size(), 3)
	assert_true(game.execute_command(&"confirm"))
	assert_true(game.execute_command(&"vertical", {"step": -1}))
	assert_eq(game.save_state()["mode"], 4)
	assert_true(game.execute_command(&"vertical", {"step": 1}))
	assert_eq(game.save_state()["case_states"][0]["inspected"], [true, false, false])
	assert_true(game.execute_command(&"back"))
	assert_eq(game.save_state()["case_index"], 0)
	assert_eq(game.save_state()["inspected"], [true, false, false])
	assert_true(game.execute_command(&"vertical", {"step": -1}))
	assert_true(game.execute_command(&"vertical", {"step": 1}))
	assert_true(game.execute_command(&"confirm"))
	assert_eq(game.save_state()["case_index"], 1)
	assert_eq(game.save_state()["inspected"].size(), 7)
	assert_true(game.execute_command(&"confirm"))
	assert_true(game.execute_command(&"vertical", {"step": -1}))
	assert_true(game.execute_command(&"vertical", {"step": 1}))
	assert_true(game.execute_command(&"confirm"))
	assert_eq(game.save_state()["case_index"], 2)
	assert_eq(game.save_state()["inspected"], [false, false, false, false, false])
	assert_eq(game._answer_name(0), "가장 그럴듯한 설명")
	assert_true(game.execute_command(&"vertical", {"step": -1}))
	assert_true(game.execute_command(&"vertical", {"step": -1}))
	assert_true(game.execute_command(&"confirm"))
	assert_eq(game.save_state()["case_index"], 1)
	assert_eq(game.save_state()["inspected"][0], true)

func test_violet_case_second_case_keeps_theories_unresolved() -> void:
	var game := _spawn(&"violet_case")
	assert_true(game.execute_command(&"vertical", {"step": -1}))
	assert_true(game.execute_command(&"vertical", {"step": 1}))
	assert_true(game.execute_command(&"confirm"))
	...
	assert_eq(game.save_state()["mode"], 3)
	assert_true(game.save_state()["solved"])
	assert_true(game._notebook_body.text.contains("미확정 이론"))
	assert_true(game.execute_command(&"confirm"))
	assert_eq(_requests[-2], {"kind": &"observation", "payload": {"id": "violet_case.music_box_solution", "text": "확정된 것은 가짜 경찰이 인계 기록을 꾸며 소녀를 데려갔다는 사실이다. 오르골과 소녀의 물질적 연결은 남아 있지만, 청동상·악마·무한동력에 관한 설은 아직 닫히지 않았다."}})
	assert_eq(_requests.back(), {"kind": &"portal", "payload": {"exit": "forward"}})
