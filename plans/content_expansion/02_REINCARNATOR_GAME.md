...
드문 대화 선택이 있을 경우 결과는 Effect로 WorldState를 바꾸지만, 대화 선택만으로 핵심 상황을 해결하지 않는다.
...
9. optional 상호작용을 찾아다닐 이유가 새로운 물체 반응·NPC 행동·세계 변화에 있는가?
...
이 대조는 긴 설정 설명보다 물건에 대한 반응·행동 차이와 드문 짧은 대사로 보여준다.
