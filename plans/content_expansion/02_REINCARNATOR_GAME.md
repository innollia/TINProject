...
# 15. 첫 45~75분 수직 슬라이스 — 실제 콘텐츠

이 절의 목적은 "NPC 4명", "상호작용 10개" 같은 추상적 수량을 없애고 **플레이어가 실제로 무엇을 하는지**를 고정하는 것이다.

정식 이름이 없는 인물/장소는 `[역할명]`으로 표기한다.  
이 역할명은 개발 fixture이며 정본 이름이 아니다.

첫 5~8분 이후 집·마을 중심·외곽을 자유롭게 왕복한다.
...
1. **밟혀 하얘진 문지방**
   - 첫 조사: 여러 사람이 오래 드나든 흔적
   - 두 번째 조사: 왕성의 문지방과 비교하는 짧은 독백
   - 상태 변화 없음

2. **접어 둔 평민 옷**
   - 조사하면 "이 정도면 이제 자연스럽다"와 비슷한 반응
   - 왕녀가 이미 3년째 숨어 살고 있음을 설명문 없이 암시

3. **봉인된 옛 물건 상자**
   - 열 수는 있음
   - 안에는 왕녀 시절을 직접 증명할 만한 물건이 있다는 것만 확인
   - 첫 slice에서는 꺼내지 않음
   - `home.royal_box_seen` fact 추가
...
5. **문 옆 장바구니**
   - 비어 있음
   - 나중에 마을에서 물건을 담으면 실제 sprite/label 상태 변화
...
두 가느다란 도구는 UI상 처음부터 "칫솔 / 귀이개"라고 이름표를 붙이지 않는다.
...
플레이어가 한 도구를 입에 사용하면 시나가 들어와:

> "어머머 이름님! 그건 칫솔이 아니잖아요! 어쩌다 귀이개를 칫솔이랑 헷갈리신 거예요...."

주인공:

> "아으, 퉷 퉤 퉤."

> "이것이 귀이개란 말이더냐. 다 똑같이 생겨먹어선 구분을 할 수 있어야지 원."

시나:

> "구분을 할 수 없는 게 당연한 거예요 이름님, 같은 물건인걸요. 서민은 그 둘을 구분 안 한다니깐요."

주인공:

> "그럼 왜 잘만 하고 있는 양치를 막아선 게야."

시나:

> "이름님 혼자 양치하시는 모습이 기특해서 장난 좀 쳤어요."

이름은 사용자 미정이므로 실제 구현에서는 호칭 placeholder를 사용한다.

#### 분기

플레이어가 먼저 두 도구를 모두 조사했어도 장면은 발생한다.
정답 도구를 고르는 퍼즐이 아니다.

플레이어가 귀에 먼저 사용하면 시나의 첫 문장만 달라진다.

두 번째 조사 이후에는:
- 어느 쪽을 어디에 써도 시나가 새로운 장난 한 줄을 함
- 실패/정답 판정 없음
...
- 자유 이동
- 물건 집기/사용
- 같은 대상의 상태 기반 대사 variant
- 시나 NPC reaction
- 자동 컷신이 아닌 직접 행동에서 사건 발생

---

## 15.2 집 — 양치 뒤 8~12분의 선택 콘텐츠

화장실을 나오면 바로 마을로 갈 수 있다.

하지만 집 안에는 다음 선택 콘텐츠가 있다.
...
마늘을 먹은 직후 text leak을 다시 확인하면  
주인공이 아침의 문장을 떠올리지만 **의미를 해석하지 못함**.

이것은 원인 규명이 아니라 motif 반복.

### B. 시나

대화 topic은 상황에 따라 달라짐.
...
첫 slice에서는 이름/성격을 만들지 않는다.

집 안 배경 또는 짧은 존재 확인만:
- 한 명은 밖에 나가 있음
- 한 명은 자고 있음
- 한 명은 뭔가 수선 중
- 나머지도 "여섯이 함께 도망왔다"는 사실을 보여줄 정도로만 존재

**개발 모델이 임의 이름/성격을 확정하지 않는다.**

### D. 왕녀 물건 상자 + 시나 조합

상자를 본 뒤 시나에게 말 걸면:
- 시나가 문을 확인한 뒤 목소리를 낮춤
- 도시 수색이 아직 끝나지 않았다는 짧은 정보
- "그래도 이 마을에서는 이미 꽤 오래 살았다"는 반응
...
마을은 첫 방문에도 이미 "처음 보는 RPG 마을"처럼 행동하면 안 된다.
...
중요:
**감자가 적이라고 해서 상점의 감자를 자동으로 적대 생물과 동일한 것으로 확정하지 않는다.**
...
"건실한 인상을 오랫동안 쌓았다"는 사실을 수치가 아니라 이 대화에서 체감.

장바구니를 집에서 들고 왔으면 구매 물건이 실제 장바구니에 보임.
...
가 달라지는 **cross-location reaction 1**로 사용.

### 15.3.3 수호신 먹이기

작은 사당/신상.

이 종교의 목적은 사용자 아이디어 그대로:

> 많이 먹여서 수호신이 뚱뚱해지는 모습을 보고 "아 좋다" 할 뿐

#### 실제 상호작용

먹이를 바칠 때마다 신상의 외형이 단계적으로 바뀜.
...
이 상호작용은 "세계가 내가 건드린 만큼 남아 있다"는 감각을 증명.

### 15.3.4 외눈 교복 학생

벤치/담장/가게 앞 등 한 곳에 있음.
...
대화에서:
- 다른 세계의 학교를 다녔다고 주장
- 여기 학교와 다른 점을 이상하게 구체적으로 하나 이야기
- 주인공은 자신의 환생 경험과 비교하지만 **자신의 전생을 공개하지 않음**

첫 대화만으로:
- 다른 세계가 실제 존재한다고 확정하지 않음
- 학생의 말이 진실인지도 확정하지 않음

다시 말을 걸면:
- 같은 이야기를 반복하지 않고 학교 생활의 사소한 디테일 2~3개를 더 말함

이 캐릭터는 첫 slice에서 퀘스트를 주지 않는다.

### 15.3.5 수색 전단

마을 벽 한편에 오래된 전단이 있음.

너무 낡아 현재 얼굴과 바로 연결하기 어려움.
...
찢기/훼손은 첫 slice 기본 행동으로 넣지 않는다.
괜히 경찰 수배 시뮬레이션으로 게임 축이 이동하는 것을 막는다.

전단을 본 뒤 시나에게 돌아가면 대사 variant 발생.

---

## 15.4 마구간 — 공룡을 "탈것 UI"가 아니라 실제 대상으로

`[마구간 관리인]`에게 말 걸면 공룡을 탈 수 있다는 사실은 이미 주인공이 알고 있음.
...
공룡의 정확한 종/지능/문화적 위치는 미정.

### 탈것 감각

첫 구현:
- 마을 내부에서는 탑승 제한 가능
- 외곽에서 탑승하면 이동 속도와 통과 가능한 route가 달라짐

단순한 "빠른 이동 버튼"이 아니라:
- 좁은 길 못 감
- 높은 턱/진흙 등 특정 지형 반응
- NPC가 공룡 탄 상태를 보고 다른 한 줄

같은 world interaction을 최소 2개 넣는다.

### 집과의 반응

공룡을 타고 집 앞까지 왔다면 시나 반응이 달라짐.

이것이 **cross-location reaction 2**.

---

## 15.5 외곽 — 첫 실제 장애물

마을 반대편 route에 **감자 관련 적대 현상**이 길을 막고 있음.

정식 생태는 사용자 미정이므로 첫 구현에서는 `[적대 감자 현상]` fixture로 둔다.
...
공룡을 타고 접근하면 장애물의 반응이 달라져 통과 가능.

이것은 "공룡이 감자를 먹는다" 등 구체 생태를 확정하지 않는다.
단순히 **공룡 동반 상태가 해결 조건 중 하나**인 개발 fixture.

### 개발 fixture 해결법 B — 환경 상태

마을/외곽에서 바꿀 수 있는 환경 상태 하나로 다른 해결 가능.
...
정확한 소재는 구현 전에 사용자 설정과 맞춰 결정.

### 개발 fixture 해결법 C — 우회

관찰을 잘하면 좁은 샛길 발견.
공룡은 못 지나가고 사람만 지나갈 수 있음.
...
세 방식 중 둘 이상이 실제로 구현돼야 함.

### 통과 뒤

거대한 새 지역을 열지 않는다.

작은 전망 지점/도로 표지/다음 국가 방향을 볼 수 있는 곳까지.

여기서 플레이어는:
- 마사지 목표가 엄청 멀다는 걸 공간적으로 체감
- 지금 당장 그 나라에 갈 수 없음을 확인
- 다시 마을로 돌아갈 이유가 생김

---

## 15.6 돌아왔을 때 달라져 있어야 하는 것

외곽까지 보고 돌아온 후 마을을 처음 상태 그대로 두면 실패.

최소 변화:

1. 공룡 탑승 여부에 따라 마구간/주민 대사 변경
2. 우물 상태를 건드렸다면 주변 주민 위치/대사 변경
3. 수호신에게 먹이를 줬다면 외형 그대로 유지
4. 수색 전단을 봤다면 시나 대사 추가
5. 외눈 학생에게 다시 말 걸면 첫 대화 이후의 후속 이야기
6. 외곽 장애물을 해결했다면 `[마구간 관리인]` 또는 `[우물 옆 주민]`이 그 사실을 간접적으로 언급

플레이어가:
> "내가 갔다 온 사이 게임이 기억하고 있네"

를 느껴야 함.

---

## 15.7 개발용 "예상 밖 목표 달성" 검수 시나리오

정식 인생목표는 사용자가 추가하기 전까지 만들지 않는다.

하지만 핵심 게임 감각을 검수하기 위해 **개발 빌드 전용 goal** 하나를 둔다.

예시 fixture:

```text
[DEV ONLY] 어떤 사람에게서 받은 물건을,
그 사람을 다시 만나지 않은 채 원래 장소로 돌아오게 하기
```

의도된 테스트 흐름은 하나가 아니다.

상호작용 여러 개를 하다 world state가 우연히 조건을 만족하면 갑자기:
...
이 fixture의 문구/조건은 출시 전에 삭제.

검수 목적은 오직:
**"퀘스트 수행"이 아니라 다른 일을 하던 중 인생목표가 사후적으로 맞아떨어지는 쾌감** 확인.

---

## 15.8 개발용 기억 해금 위기 검수

정식 위기 사건도 아직 사용자 미정.

개발 빌드에서만 다음 구조를 가진 fixture를 만든다.

### 상태

플레이어가 서로 무관해 보이는 세 행동을 함:
...
세 조건이 모두 성립한 순간에도 위험 경고 UI 없음.

### 발동

다음 location transition 직전에:
- 화면/소리 변화
- 봉인된 기억 fragment가 자동으로 1개 나타남
- 주인공이 내용보다 먼저 **"왜 지금 이게 떠오르지?"**라고 반응
- 그 뒤 현재 상황의 공통점을 알아차림

그제야 새 interaction variant가 생겨 위기를 피할 수 있음.

fixture 내용은 release canon이 아님.

---

## 15.9 첫 slice에서 실제로 소비 가능한 분량
...
빠른 플레이어는 20분 안팎에 핵심만 보고 다른 TIN 게임으로 빠질 수 있고,
관찰하는 플레이어는 60분 이상 머무를 수 있게 한다.

---

## 15.10 첫 slice 완료 게이트

코드가 실행된다는 것만으로 완료 금지.

실제 플레이 빌드에서 반드시 확인:

- 양치 장면이 플레이어 행동으로 시작됨
- 집→마을→외곽→마을 왕복 가능
- 수호신 외형 단계 0~4 persistence
- 외눈 학생 대화 최소 3회가 같은 문장 반복이 아님
- 공룡 탑승 전/후 상호작용 차이
- 외곽 장애물 실제 해결 경로 2개 이상
- cross-location reaction 2개 이상
- 돌아온 장소가 눈에 띄게 달라짐
- 개발용 goal이 의도하지 않은 맥락에서 완료되는 감각 확인
- 개발용 crisis가 "기억이 먼저 풀려 위험을 인지"하는 순서를 지킴
- save/load 후 위 상태가 그대로 복원
- 정식 미정 설정이 fixture에서 canon 문서로 새지 않음
...
이 절은 시스템 아이디어가 아니라 **실제로 분량을 채울 authored content 후보**다.
...
플레이어가 이미 3년 동안 해온 생활의 **구체적인 흔적**을 마을 곳곳에서 확인하고,
현재 플레이 중에도 몇 가지 약속을 직접 지킨다.
