...
| 항목 | 값 |
|---|---|
| 내부 버퍼 | 논리 960 × 640 안전 영역 (16:9 는 논리 폭 1138). 실제 픽셀 크기로 래스터화 |
| 정수 배 | 쓰지 않는다 (2026-09-27 부터 실제 해상도 래스터) |
| 시뮬레이션 | 30Hz 고정 틱 |
...
| 스크립트 언어 | **없다** (사용자 확정) |
| 테스트 | Kit core 52 · 비주얼 계약 15 · 장면 15 (합 82). 프로젝트 전역은 2026-09-26 644 가 마지막 실측 |

---

## 4. 다음 작업 (순서 고정)

1. ~~A1 씬 팔레트 authored 고정~~ — 2026-09-27 완료 (추천안)
2. ~~A2 구조물 기하를 명시적 폴리곤으로~~ — 2026-09-27 완료 (추천안)
3. **A3 개체 — 사용자 반려. Rain World식 절차 애니메이션으로 다시 한다** — `17` §A3 재개 · `IMPLEMENTATION_STATUS` §5
4. ~~소품(prop) 시스템~~ — 2026-09-27 완료 (V9 규칙 깨기 1개)
5. ~~V13 4해상도 겹침 실측~~ — 2026-09-27 완료 (16장 겹침·잘림 0)
6. `02`/`03`/`06`/`13` 계획 문서를 4속성 · 로비 구조에 맞춰 갱신

지금 할 일은 3 이다. 시작 전에 `17` §A3 재개의 질문 2개를 사용자에게 번호 붙여 묻는다.
**추측으로 착수하지 않는다.**
