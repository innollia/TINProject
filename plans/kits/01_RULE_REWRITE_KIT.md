...
콘텐츠 단위는 **독립 퍼즐 보드**다. content/index.json은 순서와 파일 경로만 갖고, 각 board JSON은 schema_version, stable id, width/height, 시작 entity 목록, 표시 제목, 3D 시작 여부, 완료 조건과 사용할 독자 아트 recipe ID를 가진다. 원작 레벨 배치는 시스템 검증과 수치 측정 목적으로 복사할 수 있다. 복사한 보드는 검증 입력으로 기록하고 authored content 완성 증거로 세지 않는다. 로더는 명사·word 역할·ID 중복·셀 범위·DOOR 경계 메타데이터를 검증한다. 새 board는 JSON과 인덱스 한 줄만 추가하고 parser/movement/save/registry의 알고리즘은 건드리지 않는다.

Reference Game은 아래 **16개 보드**를 처음부터 바로 연속 플레이한다. 01–15는 authored 보드이고 16은 원작 배치 검증본이라 authored content 완성 증거에서 제외한다. 각 보드는 이미 아는 규칙을 빠르게 결합하는 과제이며 힌트 발견·대기·긴 이동으로 시간을 채우지 않는다. 개별 보드의 예상 45–90초는 작성 가설이고 완료 조건은 실제 처음부터 끝까지 10분 이상 측정한 기록이다. 10분 미만이면 서로 다른 조합 보드를 추가하거나 기존 보드의 유의미한 상호작용을 보강한다. 대안 route와 우회는 허용하되, 특정 문장 집합을 강제로 완성시키는 trivial한 puzzle이나 핵심 시스템과 무관한 우회만 남는 콘텐츠는 REFERENCE GAME 증거로 삼지 않는다.

각 보드의 유한 최적 해와 깊이별 해 개수 곡선은 `tools/rule_judge/`에서 측정한다. 유한 해가 없으면 `UNSOLVED`로 기록하고 추측으로 메우지 않는다. 이 측정값은 캠페인 게이트가 아니라 다음 저작 판단의 근거다.

2026-09-26 첫 측정 결과 ([상세 기록](../../docs/research/rule_rewrite/BOARD_JUDGE_REPORT_2026-09-26.md)):

- **10번은 규칙을 쓰지 않는다.** 상태 공간 4,023개 전수 열거, 최단 해 5턴이며 `RRRRR` 다섯 회가 유일한 해다. 규칙 단어를 한 번도 밀지 않는다. §7 표가 적은 "공유 벽의 두 방" 과제와 무관하다.
- **12번도 동일**하게 6턴 `RRRRRR`, 규칙 변경 0회.
- 01~08번은 `board_fill` 0.13~0.21로 보드의 79~87%가 비어 있다.
- 03·05·06·08·13·16은 보드 전체가 이동 가능reachable하다(공간 제약 0). 난이도가 전부 규칙에서 나와야 하는데 이 보드들의 규칙 수는 3~5개다.
- 16번은 도달 가능 상태 1,240개를 깊이 27까지 전부 열거했고 승리 상태가 없다. `BABA IS MELT`가 왼쪽 벽에 묶여 끊을 수 없고 `BASIN IS HOT` 열을 반드시 건너야 해서 전멸한다. 원작의 해법은 확인하지 못했다.
- 11개 보드는 노드 예산에 닿은 상태이며, 예산 내 미발견은 불가능 증명이 아니다. 판정선 수치는 원작 퍼즐 데이터 확보 후에만 기록한다.

원작 로직 대조에서 드러난 미결 사항: 파괴 속성 우선순위(`WEAK` > `DEFEAT`)를 원작에 맞게
수정하는 과정에서 **board 06의 저작 루트가 무효가 되었다.** `VINE IS DEFEAT AND WEAK`에
닿은 LARK는 원작 규칙에서 살아남기 때문에 `MOTH HAS KEY`가 발동하지 않는다.

사용자 결정이 필요한 모든 항목(위 사항을 포함한 A~F 분류)은
[미결 질문 목록](../../docs/research/rule_rewrite/OPEN_QUESTIONS.md)에 있다. 그 문서에서
결정되면 이 계획서의 수치와 보드 목록을 갱신한다. **보드에 맞춰 테스트를 고쳐 통과시키는
선택지는 목록에 넣지 않았다.** 그건 규칙이 아니라 숫자를 맞추는 작업이고, 원작 로직
대조 작업의 취지를 배반한다.

상세 근거: [원작 로직 대조 기록](../../docs/research/rule_rewrite/ORIGINAL_LOGIC_DIVERGENCES.md),
[보드 판정 측정 기록](../../docs/research/rule_rewrite/BOARD_JUDGE_REPORT_2026-09-26.md).
...
| 15 | METRIX/INV와 WORD, 3D, OWNS/ACTIVE를 결합해 두 보관실 중 하나를 선택해 WIN을 완성하기 | 09–14의 데이터/도메인 재사용 |
| 16 | 원작 배치 검증본. 같은 주어에 FLOAT와 MELT가 겹친 규칙 상태에서 겹침을 끊어 WIN 접촉을 만든다 | 01–15의 문장 조작 |
...
기존 at-icons 월드 자산과 그 제작 recipe는 보존하되, 해당 조립 기준은 2026-09-25부터 현행 제작 지침이 아니다. 대체 이미지 자산 기반이 확정되면 캐릭터, BOX/DOOR, ROCK, 목표물의 출처·표현·라이선스와 2D/3D/인벤토리 미리보기 간 물리 entity ID 유지 방식을 이 계획에 반영한다. 글자는 단어 타일로 읽히게 자체 타이포그래피를 사용한다. 기존 이미지 파일은 현재 경로와 내용 그대로 둔다.
...
6. **2D/3D 표현과 입력**: 사용자 캡처와 E01–E03를 대조하며 보드·실패 안내·3D 투영·핫바·인벤토리를 붙인다. 월드 art 제작 기준은 대체 이미지 자산 기반 확정 뒤 갱신한다.
7. **16개 보드 저작과 측정**: 앞 보드의 문법이 뒤 보드에서 다시 쓰이는지 확인하고, 데이터 추가만으로 마지막 보드를 넣는다.
...
12. `tools/rule_judge/`로 각 보드의 유한 최적 해와 깊이별 해 개수를 측정한다. 예산 초과나 해 부재는 `UNSOLVED`로 기록하고 수치를 지어내지 않는다. 판정기의 결과가 실제 게임과 같은지는 `tests/core/test_rule_judge_parity.gd`가 게이트로 확인한다(15개 보드 × 16턴). 그 테스트가 어긋나면 판정기 수치는 버린다.

수동 플레이는 보드 01부터 16까지 설명문 없이 연속 진행한다. 관찰자는 첫 유효 입력까지 시간, 오조작, 규칙 변화 인지, 실수 후 복귀, 3D 인벤토리와 보드의 동일성 이해를 기록한다. 11에서 문으로 들어가 내부 물건을 옮기고, 10에서 두 METRIX를 동시에 움직이고, 12에서 안쪽 벽 정렬을 눈으로 확인하고, 13에서 ROCK을 배치해 문장을 완성하고, 14에서 소유/ACTIVE 규칙을 깨고 복구하며, 15에서 두 authored route 중 하나를 선택해 데이터 확장 결과를 확인하고, 16에서 같은 주어에 겹친 두 규칙의 충돌을 확인한다. 10분 이상은 실제 시간을 재서 기록한다.
...
금지: 세 단어 IS 파서에 레벨별 예외를 덧대기, 첫 유효 BOX 사각형만 고르기, 인벤토리 UI에 아이템 사본 만들기, 3D를 독립 게임으로 만들기, 물리 아이템의 2D↔3D 전환에서 ID 교체, ASCII/ColorRect+Label 월드 보드, 상시 규칙표·키 설명·Shell HUD, 원작 자산 복제, 라이선스 미확인 모드 코드 복사, 새 board마다 core 분기 추가.

검토 준비 완료의 제출물:

- 원작 자료 ID와 TIN 캡처를 상태별로 나란히 놓은 비교표.
- 16개 authored board의 데이터와 **실측 10분 이상** 연속 플레이 기록.
