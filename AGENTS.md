...
- 다른 세션에 나눠 맡길 때도 같은 범위를 적용한다. 스킬의 “사실을 조사하라”는 지시는 현재 설계 선택에 필요한 사실에만 적용하며, 불필요한 코드 조사를 새로 만드는 근거가 아니다.
...
- 각 지역의 동선·생활·물건·NPC 배치·적·사건·변화 등 전체 정보는 지역별 독립 담당이 책임진다. 각 캐릭터의 외형·대사·행동·관계·상호작용은 캐릭터별 한 독립 담당이 책임진다. 독립 담당은 사용자가 따로 여는 대화 세션이며 서브에이전트로 띄우지 않는다(아래 '병렬 세션 작업'). 서로 다른 담당자가 같은 정본을 동시에 고치지 않는다.
...
- 2026-09-26 사용자 결정: 판정을 기다리던 생성형 도구 화풍 시험(Nano Banana Pro/Seedream 4.5 비교, Layer GPT Image 2, 내장 imagegen A 단독 등)은 전부 불합격이다. 04 Top-down Action-RPG Kit는 코드로 그린 그림(아이콘 조합, 도트, SVG/Pillow 절차 그림)을 최종 그림으로 허용한다. 04의 코드 그림에는 아래 GPT 출력·기계적 후처리 한정 줄과 생성기 화풍 게이트를 적용하지 않는다. 그림을 만든 스크립트·SVG 원본을 결과 옆에 두고, 결과는 candidate로 시작하며 승인·Gold Standard 승격은 사용자만 한다. 같은 날 사용자 결정으로 아이콘 조합의 코딩 금지도 해제한다. 04에서는 스크립트·코드로 아이콘을 배치·변형·합성해도 되며, `archive/icon_based_image_assets/`의 편집기 전용·코드 생성 금지는 적용하지 않는다.
...
2026-09-27 사용자 지시: 남은 작업은 여러 대화 세션이 동시에 나눠 한다. 서브에이전트·워크플로(`spawn_run`, `spawn_sub_agents`, `workflow_run`)는 쓰지 않고 각 세션이 자기 대화방에서 직접 한다.

- 모든 세션이 같은 작업 폴더와 같은 브랜치(`kit/05-stone-story-rpg`)를 쓴다. 브랜치 전환·`stash`·`reset`·`checkout`·`pull`·`rebase`·`merge`·`clean`을 하지 않는다.
- 세션은 첫 지시에 적힌 소유 경로만 쓴다. 그 밖은 읽기만 한다. 밖의 변경이 필요하면 자기 문서에 연결 요청으로 남기고 사용자에게 알린다.
- 공용 문서(`AGENTS.md`, `CONTEXT.md`, `PROJECT_DECISIONS.md`, `HANDOVER.md`, `plans/kits/INDEX.md`, 루트 `docs/GRILLING_STATE.md`)와 `app/**`, `project.godot`, `addons/**`, `tests/run_tests.gd`는 병렬 세션이 고치지 않는다. 새 사용자 결정은 자기 Kit 상태 문서에 적고 '정본 반영 대기'로 표시한다.
- 커밋은 `git add -- <내 경로>` 다음 `git commit -m "<메시지>" -- <내 경로>`로 한다. `git add .`·`git add -A`·`git commit -a`는 금지다. 다른 세션이 올려 둔 staged 파일이 섞이지 않게 하기 위해서다. 푸시는 `git push origin kit/05-stone-story-rpg`. `index.lock`이나 푸시 거절은 잠시 뒤 다시 한다.
- Godot은 한 번에 한 세션만 돌린다. 잠금 파일을 만든 세션만 실행하고 끝나면 지운다. 파일이 이미 있으면 기다린다. 40분 넘게 남아 있는데 godot 프로세스가 없으면 남은 잠금으로 보고 지운다.

```powershell
New-Item -ItemType Directory -Force C:\projects\_locks | Out-Null
New-Item -ItemType File -Path C:\projects\_locks\TINProject-godot.lock -Value '<세션 이름>' -ErrorAction Stop  # 실패하면 기다렸다 다시
# Godot 명령들
Remove-Item C:\projects\_locks\TINProject-godot.lock
```

- 작업 중에는 자기 테스트만 돌린다. 아래 전체 자동 검증은 마무리 때 한 번 돌리고, 남의 소유 파일에서 난 실패는 고치지 않고 기록한다.
- 사용자 결정이 필요하면 카드만 보내지 말고 대화 본문에 번호를 붙여 묻는다.
