...
def load_palette(path) -> dict:
    """mp05 addition: a palette may name a base palette (same folder) in ``extends``.
    The base colors / materials / styles are deep-merged under this file, so a
    derived palette only lists what it adds or changes."""
    path = Path(path)
    data = load_json(path)
    base = data.get("extends")
    if base:
        data = deep_merge(load_palette(path.parent / base), {k: v for k, v in data.items() if k != "extends"})
    return data
...
def translate_forms(forms: list, dx: float, dy: float) -> None:
    """mp05 addition: move every placement by (dx, dy), repeats and vertical gradients included."""
    for form in forms:
        if form.get("pivot"):
...
def frame_forms(recipe: dict, frame: dict) -> list:
    ...
    off = recipe.get("offset_all")
    if off:  # mp05: applied after frame moves, so frame pivots stay in recipe coordinates
        translate_forms(out, float(off[0]), float(off[1]))
    if frame.get("mirror"):
