...
def palette_bytes(path: Path) -> bytes:
    """Palette file bytes plus its ``extends`` chain (mp05), so editing a base palette rebuilds."""
    data = path.read_bytes()
    base = json.loads(data.decode("utf-8")).get("extends")
    return data + (palette_bytes(path.parent / base) if base else b"")
