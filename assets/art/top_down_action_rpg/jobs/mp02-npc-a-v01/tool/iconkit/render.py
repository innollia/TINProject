...
class Renderer:
    ...
    def render(self, recipe: dict, frame: dict | None = None) -> dict:
        ...
        if sil:
            if sil.get("edge_extend"):
                # mp02: a bust cut by the canvas edge must not get an outline along that edge.
                # Pad the alpha with its own border values so the canvas edge is not an edge.
                p = int(math.ceil((sil.get("width", 1.4) + sil.get("heavy", 1.0)) * ss)) + 2
                padded = np.pad(canvas.a, p, mode="edge")
                band = P.edge_band(padded, sil.get("width", 1.4) * ss, sil.get("heavy", 1.0) * ss,
                                   self.light2)[p:-p, p:-p]
            else:
