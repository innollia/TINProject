...
def main() -> int:
    ...
    ap.add_argument("--label", default="#2a1a2e", help="label colour (mp02: readable on the dark V1 floor)")
    args = ap.parse_args()
    ...
    for s, rh in zip(scales, row_h):
        ...
        for im, sh, p, cw in zip(images, shadows, paths, col_w):
            ...
            draw.text((x, y + size[1] + 2), f"{p.stem} x{s:g}", fill=args.label, font=label_font)
            x += cw
        y += rh
