...
def load_palette(rdir, name: str, _depth: int = 0) -> dict:
    """mp08: a palette may say ``"extends": "palette_h0_mood.json"``; it is deep-merged
    over that base, so a region palette only lists what it changes."""
    data = load_json(Path(rdir) / name)
    base = data.pop("extends", None)
    if base and _depth < 4:
        data = deep_merge(load_palette(rdir, base, _depth + 1), data)
    return data


def palette_bytes(rdir, name: str, _depth: int = 0) -> bytes:
    """Raw bytes of a palette and every palette it extends (for build keys)."""
    raw = (Path(rdir) / name).read_bytes()
    base = json.loads(raw.decode("utf-8")).get("extends")
    if base and _depth < 4:
        raw += palette_bytes(rdir, base, _depth + 1)
    return raw
