...
def scale_forms(forms: list, factor: float, origin) -> None:
    ...
    for form in forms:
        ...
        grad = form.get("grad")
        if grad:  # mp03: gradient rows are canvas y values, so they scale with the figure
            grad["y0"] = oy + (grad["y0"] - oy) * factor
            grad["y1"] = oy + (grad["y1"] - oy) * factor
        for piece in form.get("pieces", []):
...
def translate_forms(forms: list, dx: float, dy: float) -> None:
    """mp03: shift every placement by (dx, dy) canvas px (``scale_all.to``).

    Grid repeats start at the piece's ``at`` (already moved) and their ``step``
    is relative, so only absolute repeat anchors are shifted here.
    """
...
def frame_forms(recipe: dict, frame: dict) -> list:
    ...
    if sa:
        # mp03: {"factor", "origin", "to"}: scale around origin, then move origin onto "to".
        # With "to" the origin must be given in the authoring frame (e.g. the player's
        # 192x192 frame, feet at [96, 186]); "to" is the output canvas pivot.
        origin = sa.get("origin", recipe.get("pivot", [0, 0]))
        scale_forms(out, float(sa.get("factor", 1.0)), origin)
        if sa.get("to"):
            translate_forms(out, sa["to"][0] - origin[0], sa["to"][1] - origin[1])
    if frame.get("mirror"):
...
class Renderer:
    def __init__(self, cache_dir=None):
        ...
        self._stamps: dict = {}
        self.instances = 0
        self.stamp_scale = 1.0  # mp03: set per recipe from style["stamp_scale"]
    ...
    def _stamp(self, spec: dict, ss: int) -> Image.Image:
        key = json.dumps(spec, sort_keys=True) + f"|{ss}|{self.stamp_scale}"
        if key not in self._stamps:
            ...
            if box:
                img = img.crop(box)
            # mp03: style ``stamp_scale`` enlarges brush stamps for bigger-than-sprite
            # assets (portraits) so strokes keep the same on-screen size.
            length = max(2, int(round(spec.get("length", 12) * ss * self.stamp_scale)))
            width = max(2, int(round(spec.get("width", 3) * ss * self.stamp_scale)))
    ...
    def _stain_stamps(self, spec: dict, ss: int) -> list:
        key = "stain|" + json.dumps(spec, sort_keys=True) + f"|{ss}|{self.stamp_scale}"
        if key in self._stamps:
            return self._stamps[key]
        size = float(spec.get("size", 40)) * ss * self.stamp_scale    # mp03: style stamp_scale
        soft = float(spec.get("soft", 0.0)) * ss * self.stamp_scale
    ...
    def render(self, recipe: dict, frame: dict | None = None) -> dict:
        ...
        self.stamp_scale = float(style.get("stamp_scale", 1.0))  # mp03
        ...
        if sil:
            sw, sh = sil.get("width", 1.4) * ss, sil.get("heavy", 1.0) * ss
            if sil.get("pad_edges"):
                # mp03: a figure cut by the canvas edge (a portrait bust) must not get an
                # outline along that edge, so the alpha is extended past the border first.
                pe = int(math.ceil(sw + sh)) + 2
                band = P.edge_band(np.pad(canvas.a, pe, mode="edge"), sw, sh, self.light2)[pe:-pe, pe:-pe]
            else:
                band = P.edge_band(canvas.a, sw, sh, self.light2)
