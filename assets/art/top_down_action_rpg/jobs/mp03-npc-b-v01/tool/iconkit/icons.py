...
ICON_UNITS = 16.0
DEFAULT_RES = 1024
MAX_WORKERS = 2  # mp03: parallel SVG renderer calls (shared machine)
INKSCAPE_CANDIDATES = [
...
class IconLibrary:
    ...
    def prefetch(self, names: list[str], workers: int = MAX_WORKERS) -> None:
        """Render missing cache files in parallel (renderer calls are subprocesses).

        mp03: never more than MAX_WORKERS renderer processes, because ten art
        sessions share this machine.
        """
