        ...
        for form in forms:
            ...
            if kind == "flat":
                ...
                if grad:
                    ...
                    t = np.clip((yy - grad["y0"]) / max(grad["y1"] - grad["y0"], 1e-6), 0.0, 1.0)
            ...
            if kind == "block":
                ...
                top_shade = deep_merge(shade, form.get("top_shade", {"bump": 0.35, "ragged": 0.12,
            ...
            if grad:
                yy = (np.arange(M.shape[0], dtype=np.float32) + y0) / ss
                t = np.clip((yy - grad["y0"]) / max(grad["y1"] - grad["y0"], 1e-6), 0.0, 1.0) ** grad.get("curve", 1.0)
