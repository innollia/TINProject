...
mp01 (mp01-props-common-v01) addition: form ``decal: true`` draws a floor mark
(chalk, stain) into its own layer that skips the outer silhouette and is laid
UNDER the finished object.  Recipes without ``decal`` render exactly as in v02.
"""
...
class Renderer:
    ...
    def render(self, recipe: dict, frame: dict | None = None) -> dict:
        ...
        decals = P.Buffer(Ws, Hs)  # mp01: floor marks (chalk, stains) laid under the object, no silhouette
        main = canvas
        stored: dict = {}
        ...
        for form in forms:
            ...
            canvas = decals if form.get("decal") else main
        ...
        if decals.a.max() > 0.0:  # mp01: decals go UNDER the outlined object (premultiplied over)
            inv = 1.0 - canvas.a
            canvas.rgb = canvas.rgb + decals.rgb * inv[..., None]
            canvas.a = canvas.a + decals.a * inv
