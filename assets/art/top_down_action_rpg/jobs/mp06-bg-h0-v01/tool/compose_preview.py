...
def main() -> int:
    ...
    if bg_man_path.exists():
        ...
        # mp06: the background's own contact shadows / wall AO are built as a separate
        # _shadow.png (shadow_output "separate"); lay them on the floor before any item.
        if bg_man.get("shadow_file"):
    ...
    # mp06: fixed foreground occluders (full-canvas transparent PNGs, same origin as the
    # background).  Their floor shadow goes down with the background's; the images are
    # drawn over every item at the end.
    foregrounds = []
    for fg_rel in scene.get("foreground", []):
        fg_path = JOB / fg_rel
        fg_man_path = fg_path.with_suffix(".json")
        fg_man = json.loads(fg_man_path.read_text(encoding="utf-8")) if fg_man_path.exists() else {}
        if fg_man.get("shadow_file"):
            canvas.alpha_composite(Image.open(fg_path.parent / fg_man["shadow_file"]).convert("RGBA"), (0, 0))
        foregrounds.append((Image.open(fg_path).convert("RGBA"), fg_path, fg_man))
    entries = []
    ...
    for fg, fg_path, fg_man in foregrounds:
        canvas.alpha_composite(fg, (0, 0))
        emit.occlude(fg, (0, 0))
        if fg_man.get("emit_file"):
            emit.add(Image.open(fg_path.parent / fg_man["emit_file"]).convert("RGBA"), (0, 0))
