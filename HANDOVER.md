...
| **신규 구현 모듈** | 14개: 사이클 1·2의 12개 + 사이클 3 `numberless_clock`, `shadow_ferry` |
| **신규 시스템** | 기록 v1(전역 관찰·수동 정리·테마 수집), 죽음·귀환·지름길 검증, 프로필·체크포인트, 공용 메뉴/저널/클리커 |
| **검증 상태** | `import` → 통합 러너(639/639) → GUT(74/74, 3,014 assertions) → smoke — **전부 통과(종료 코드 0, SCRIPT ERROR 0)** |
...
> 8. 기존 데모 3개·639 checks·GUT 74/74 **회귀 0** 유지.
