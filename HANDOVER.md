...
> **2026-09-21 방향 교정:** 이 문서의 25개 신규 모듈 설명은 현재 구현 상태 기록일 뿐 콘텐츠 방향의 정본이 아니다. 대부분을 완성 게임이 아닌 프로토타입 조각으로 취급한다. 새 콘텐츠 작업 전 `PROJECT_DECISIONS.md` 0절과 `docs/KNOWLEDGE_BASED_DESIGN.md`를 우선 읽는다. 과거의 모듈 증산·100개 로드맵·준호 이름·signal_desk 반복 귀환은 향후 설계 기준으로 사용하지 않는다.
---
...
다음 작업자는 새 미니게임을 즉시 증산하지 않는다. 먼저 `PROJECT_DECISIONS.md` 0절을 기준으로 기존 프로토타입을 어떤 실제 게임 모듈에 흡수할지 설계하고, 지식 기반 요소는 `docs/KNOWLEDGE_BASED_DESIGN.md`를 통과시킨다.
