...
과거 at-icons 기반 제작 문서와 실험은 `archive/icon_based_image_assets/`에 보관한다. 이미 만든 이미지 파일은 현재 위치와 내용 그대로 둔다. 아카이브의 조립 방식과 도구는 새 이미지 제작에 사용하지 않는다.
...
A/B는 영구 Style Reference로 유지하되 권한을 제한한다. A는 안면 능선과 얼굴 명암 면, 피부·큰 색면의 내부 붓결, 머리카락 덩어리의 회화적 밝기 변화를 맡는다. B는 선 굵기 강약, 의상 구조선, 실루엣 정리, 인물/배경 밀도 대비와 배경 덩어리의 콜라주성을 맡는다. 얼굴형, 헤어스타일, 구체 모티프, 의상 디자인과 정확한 색 조합은 가져오지 않는다.
...
규칙 충돌은 `최신 사용자 결정 → 프로젝트 아트 층 → 개인 화풍 코어 → 역할이 제한된 A/B Style Reference → 해당 자산군 Gold Standard` 순으로 해소한다. Gold Standard는 환경/배경, 게임플레이 스프라이트, UI, 리깅용 전신 파츠, 비주얼노벨 전신·초상, 조사 화면으로 나누고 필요한 묶음만 활성화한다.
