...
### 11.2 레퍼런스 고유 형질 금지 — 그리고 "금지"가 아니라 "지배 금지"

**用水 규칙 (사용자 정정).** 물은 금지되지 않는다. 다른 Kit에 물·침수가 있어도 된다.
**문제는 그것이 메인이면 안 된다는 것이다.** 이전 판의 "물 금지"는 과했다.

정확한 규칙:
- 물은 **재밌을 수 있다.** 레퍼런스의 시스템과 마찬가지로.
- 그러나 이 세계관의 **정의적 형질**이 되어서는 안 된다. 첫 화면의 실루엣이 침수 폐허여서 안 되고, 세 Kit 중 어느 것도 "물 생존"을 축으로 삼아서도 안 된다.
- 판단 기준: **물이 그 장면의 조건인가, 그 장면의 무대인가.** 무대는 되지만 조건이 되면 실패다.
- 08(`descent_exploration`)은 하강 수단이 바다여도 된다. 스위밀의 시스템(하강·수집·소모·3엔딩·비월드)은 그대로 이식하고, 레퍼런스 고유 자산은 버린다.

`den`/계승은 형질이 아니라 시스템이므로 허용한다. 11.1c 참조.

### 11.1d 통행료 — 합이 아니라 최대값 병합 (W0 결정)

scale 전이는 가역이지만, rung을 넘을 때마다 `body.wounds`에 흔적이 남는다.
흔적은 `body.wounds` 안에서 **동일 정체(`part`+`kind`)끼리 합이 아니라 최댓값으로 병합**한다.

이유: 상수를 조정해서 자기모순을 없애려 하면(0.34 → 0.25) 값이 왕복 횟수에 따라 무의미해진다.
병합 규칙을 박으면 **통행료는 존재하면서 유계**가 되고, 왕복 몇 번을 하든 같은 흔적이 된다.
가역성과 영구성이 충돌하지 않는다.

### 11.1e `facts["scale"]` 우회 폐쇄 (W0 결정)

`body.facts["scale"]`은 최상위 `scale`을 우회해 사다리 검사를 깨는 두 번째 통로다. **폐쇄한다.**
`facts` 안에 `scale` 키는 존재할 수 없으며, 쓰면 `key_unknown`으로 거부한다.
몸의 크기는 `body.scale` **한 곳에만** 존재한다. 크기의 진실이 두 개면 절차 비주얼의 다섯 개 판독자가 전부 오독합니다.
