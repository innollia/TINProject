...
- 사용자가 제공한 A·B는 게임 전체의 아트 기준, C는 Rule Rewrite의 DOOM식 1인칭 카메라 기준으로 기록했다. `IS 3D` 진입은 1인칭, 키로 기존 3인칭 시점 전환이라는 결정도 기록했다. **현재 실행 화면의 3D는 기존 3인칭이며, 1인칭 기본 시점과 전환 키는 아직 구현·검증하지 않았다.** 1인칭 이동 방식은 사용자 응답을 기다리는 항목이다.
