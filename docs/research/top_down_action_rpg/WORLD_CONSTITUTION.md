...
상태: 기존 콘텐츠 초안. 2026-09-26 사용자 재기획 인터뷰 진행 중이며, 아래 기존 세계·인물·사건은 새 콘텐츠의 승인 정본이 아니다. 기존 Kit는 제작 기반으로 사용한다.
Primary Reference: BLACK SOULS 2 하나.  
원작 skin 복제 금지. 앨리스 제외.

## 2026-09-26 재기획 우선 결정

현재 결정과 질문은 [Kit 전용 인터뷰 기록](../../../plans/kits/04_TOP_DOWN_ACTION_RPG_KIT/docs/GRILLING_STATE.md)에 둔다. 이 절과 인터뷰 기록의 사용자 결정이 아래 구초안보다 우선한다.

- 목표 정서는 역겨움이다. BLACK SOULS 2의 실제 사건·인물 행동을 조사하며, 에이전트가 임의로 순화한 정서 선택지를 기준으로 삼지 않는다.
- 제작자는 세계의 역사·인과·관계를 먼저 완성하고, 게임에 등장할 문서·대사·맵의 일부를 의도적으로 훼손·삭제한다. 전모의 복원 불가능함을 허용한다. 게임 내 세계관 해설은 목표가 아니다.
- 애정과 로맨스는 유지하되 생존 보장이 아니다. 실수와 사건 진행으로 관계 상대가 죽을 수 있다. 죽음을 전부 강제하거나 관계를 전부 배신으로 만드는 규칙도 아니다.
- 구초안 R10의 모든 detail 연결 강제, R13의 신체 공포를 identity/system 기능으로만 한정하는 조건은 새 콘텐츠에 적용하지 않는다. 역겨움과 흔적을 남기는 표현을 추상적 논점이나 시스템 효용으로 정당화하도록 강제하지 않는다.
- 다크소울 스타일의 정보 훼손·단편적 전달은 사용자 지정 적용점이다. Primary Reference는 BLACK SOULS 2 하나로 유지한다.
