...
작성: 2026-09-27. 이 문서가 Kit 04 구현의 **현황 정본**이다.
...
**자동 게이트 전부 통과. 사용자 플레이 검토 전 단계다.**

`AGENTS.md` 기준으로는 "검토 준비 완료"까지만 선언 가능하다. 실제 플레이타임 측정과 720p/FHD/QHD 픽셀 증거는 아직 없다(§5).
...
|---|---|
| import | exit 0 |
| `run_tests.gd` | **644 / 644 passed** |
| Kit 04 core GUT | **19 / 19**, 7076 asserts |
| Kit 04 module GUT | **20 / 20**, 4966 asserts |
| smoke 180f | exit 0 |
| playthrough probe | **exit 0**, assertions 54 / failed 0, events 86 |
| visual capture | **exit 1** (§5) |
...
`tests/core` 전체 GUT에는 **Kit 04 외 무관 실패 5건**이 남아 있다(소유권 밖이라 건드리지 않음): `test_rule_combat_integration`, `test_rule_screen_art_links`, `test_rule_route_content`, `test_rule_ui_art_links`, `test_stone_story_rpg_core`(4 script error, `art/stone_rpg_art_view.gd:1068`).
...
## 5. visual capture harness — 미완

**이 harness는 이번 세션 전까지 한 번도 끝까지 실행된 적이 없다.** 정적 감사와 상태 2개까지만 도달한다.
...
```
금지 항목 실재 없음(placeholder / 상시 AP 게이지 / world standin / 임의 빨간 막대 / HP bar 상수 2/2)은 확인됐다.
...
원인 후보: harness의 대화 구동 로직. content 부족은 아니다 — `npc_01_ilyra_senn`가 여는 `conv_h0_return_desk`는 `pages=2 / choices=3`으로 정상이고 H0 encounter는 0건이다.

추가 결함: **실패한 run이 report 파일을 남기지 않는다.** 진단 가능한 산출물이 없다.
...
테스트: `tests/core/test_top_down_action_rpg_core.gd`(19) / `test_top_down_action_rpg_module.gd`(20) / `tests/performance/top_down_action_rpg_playthrough_probe.gd` / `top_down_action_rpg_visual_capture.gd`
