...
- **`04` §4의 통행료를 내면 그 장소에 맞는 rung으로 갈 수 있다.** 가격은
  **방향당 `0.34` 하나**이고 언제나 낼 수 있다(`04` §4.5). 그 방향의 흔적이 이미
  있으면 값이 늘지 않고 **그대로**다(`04` §4.2). "언젠가 라 돌아올 수 있다"는
...
채우는 것**이므로다. 그 이후의 모든 전환은 통행료를 낸다. **전환마다 1번**이고
`steps`와 무관하다(`04` §1).
...
| **왕복** | 안쪽으로 들어갔다가 나오는 경우 | 전환 4회 이하, **새 확정 사실 2개 이하**(방향당 1개). `04` §5.3 |
...
**통행료의 상처는 그래서 언제나 같은 값으로 읽힌다.** `04` §4.2의 `max` 병합
덕분에 그 정체의 `severity`는 `0.34`에서 움직이지 않는다(`04` §4.7). Kit이
통행료를 "몸이 얼마나 아픈가"를 잰다고 읽어도 그 수는 왕복 횟수와 무관하다.
