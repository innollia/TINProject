...
> **2026-09-26 통행료 병합 판 반영 (`ROUND_PLAN.md` §11.1d).** 추가·변경:
> **T2-16, T2-17, T4-12~T4-17, T8-17, T8-18, G23, P6d, M3(재정의).**
> `severity` 합이 `steps`나 왕복 횟수에 비례하지 않는 것을 기계로 잡는다.
...
| **17** | **`{torso, stretched, 0.34, true}` 를 쓰고 `max` 병합으로 다시 쓴 뒤 세이브/로드 → 배열 길이 1, 값 동일.** 병합은 저장이 아니라 배열 내용이다 |
| **18** | **`{torso, stretched}` 만 있는 몸에 `{torso, compressed}` 를 병합하고 세이브/로드 → 2개.** 반대 방향은 별도 정체다. `04` §4.8 |
...
| **12** | **통행료는 누적 스칼라가 아니다. `wounds` 배열에는 정체 중복이 있을 수 없다.** 임의로 만든 `{torso, stretched, 0.34, true}` 3개짜리 배열을 요청하면 스토어는 **거절하지 않는다**(재측정은 새 사실이 아니다). 그래서 12번의 실제 단언은 **`AxisBody`에 `wounds`를 순회하거나 합산하는 메서드가 0개**라는 것이며, 그것이 통행료가 세는 계기가 되지 않는 이유다. 중복 배열을 **거부할 스토어 판정을 새로 붙이지 않는다**(`S-INV-2`) |
| **13** | `missing`에 통행료가 남지 않는다. `04` §0.1. `wounds`만 남는다 |
| **14** | **전환을 아무리 반복해도 스케일 시스템이 남기는 정체가 3개를 넘지 않는다.** 6 rung 전수 쌍에서 왕복을 5회씩 반복한 뒤 `body.wounds`의 원소 수가 `≤ 2`다. `04` §4.7 |
| **15** | **`max` 병합의 세 성질이 전수에서 성립한다.** (a) **멱등**: 같은 통행료를 두 번 병합한 결과가 한 번과 같다. (b) **가환**: 같은 정체의 통행료를 `W+t` 와 `W+t'` 두 순서로 병합한 결과가 같다. (c) **결합**: 세 개를 묶는 괄호 배치가 결과를 바꾸지 않는다. `04` §4.2.2 |
| **16** | **`sum` 구현을 거짓말로 만든 버전을 이 테스트가 잡는다.** `{0.34} × 3` 성립, `{0.34, 0.80} → 0.80` 성립, `{0.34} → 0.0` 불성립(`replace` 버전을 잡는다), `{true} or {false} → true` 성립. 즉 **병합은 덧셈이 아니고 삭제도 아니다** |
| **17** | **`severity` 총량이 유계다.** 6 rung 전수 쌍 × 왕복 1·2·5·20회에서 `sum(wounds[].severity) <= 0.68` 이고, `sum`을 부르는 코드가 `AxisBody`에 0개이므로 이 판정은 테스트가 배열을 읽어 직접 한다. **`04` §4.7의 상한과 같다** |
...
**14~17번이 통행료 병합 판의 기계 증명이다.** 이 네 개는 `04` §4.2.2의 연산
정의를 그대로 옮긴 것이다. **16번이 특히 중요하다.** 합산 구현은 14번과 17번을
통과하지 않으므로 둘 중 하나만 있어도 충분하지만, 16번은 **삭제(replace) 구현**도
잡는다. `max`가 아닌 병합이 세 종류뿐이라는 것이 이 사양의 주장이다:

| 병합 | 14 | 15 | 16 | 17 | 판정 |
|---|:-:|:-:|:-:|:-:|---|
| `max` (정본) | 통과 | 통과 | 통과 | 통과 | 채택 |
| `+` (이전 판) | **실패** | **실패** | **실패** | **실패** | 기각. `04` §4.2.3 |
| `replace` | 통과 | **실패** | **실패** | 통과 | 기각. `max`가 단조 조건 |
| `min` | 통과 | **실패** | **실패** | **실패** | 기각. 통행료가 상처를 낮춘다 |
| `last_write_wins` | 통과 | **실패** | **실패** | 통과 | 기각. 순서에 의존 |
...
| **8** | **세 Kit의 소스/트리거 파일에 로컬 통행료 상수(`0.34`, `"torso"`, `"compressed"`, `"stretched"`, `"max"`) 선언이 0건** (`05` §8) |
...
| **17** | **통행료 산술이 전수에서 일치한다.** 모든 `(from, to)` 쌍에 대해 `derived_toll_kind == ("compressed" if to < from else "stretched")` 그리고 **`derived_toll_count == 1`** (이전 판의 `== steps`에서 바뀜) | `04` §4.1. 거리와 무관하게 방향당 1개 |
| **18** | **병합 규칙을 authored가 지정할 수 없다.** 세 Kit의 `triggers.json`에 `toll`, `toll_count`, `toll_kind`, `toll_severity`, `toll_merge`, `toll_mode`, `merge`, `accumulate` 키가 **0건** | `05` §5.2, `§6` `toll_rule_authored`. 병합이 닫힌 규칙이라는 증거 |
| **19** | **`wounds`를 만드는 경로는 세 Kit 모두 `scale_toll.merge_max`를 거친다.** 각 Kit 소스에서 `wounds`에 append하거나 `severity`를 더하는 코드가 0건 | `05` §5.3. `07` Q16 |
...
| **11** | **`wounds`의 개수를 읽는 reader가 0개.** `body.wounds`의 `size()`·길이·원소 개수를 시각에 반영하는 코드가 0개. 읽는 것은 정체별 **존재 여부**와 `q`에서 계산한 값뿐이다 | `04` §4.6. `max` 병합 뒤 개수는 횟수를 담지 않는다 |
...
| **G23** | **이 Kit은 통행료를 합산하지 않는다.** `modules/<kit>/**` 소스에서 `wounds`에 `append`하는 코드, `severity`에 `+`/`+=`를 쓰는 코드, 반복 횟수를 곱하는 코드가 **0건**. 그리고 **이 Kit의 병합 구현이 1개**다(공유 `scale_toll.merge_max` 호출) | `04` §4.2.2, `05` §8 |
| **G24** | **같은 방향으로 두 번 넘어가도 배열이 길지 않다.** 이 Kit의 트리거 1개로 왕복 3회를 시뮬레이션하면 `wounds` 원소 수가 1에서 시작해 2를 넘지 않고, 두 번째·세 번째 왕복에서 **배열이 `==`다** | `04` §4.2, §5.3 |
| **G25** | **이 Kit이 저작한 상처도 같은 병합을 쓴다.** 이 Kit이 `wounds`에 쓰는 모든 정체에 대해 `max` 병합이 성립하고, 합산·덮어쓰기·`min` 경로가 0건 | `05` §5.1. 정체 기반 병합은 `kind` 값과 무관하다 |
...
**G23~G25가 통행료 병합 판의 Kit 단언이다.** G24는 플레이 경로를 직접 시뮬레이션하므로
"T4-14가 스토어를 통과해도 게임에서 흔적이 쌓이면" 그 상태를 잡는다. 세 Kit이 각각
자기 병합을 갖고 있다면 G23이 그것을 잡고, 세 Kit이 공통 helper를 쓰면 G23은 통과하고
T4-15가 규칙 자체를 검증한다. **T4는 연산이 맞는지, G23은 그 연산을 Kit이 공유하는지를
본다. 둘 다 있어야 이 축이 닫힌다.**
...
| **M3** | 10분 Reference Game를 한 번 플레이한 뒤 `body.wounds`의 **내용**을 적는다. 수치가 아니라 기록의 종류와 개수를 본다 | `04` §4.7. `0.34`가 맞는지가 아니라 **병합이 게임에서 성실한지** | 기대값: `wounds` 원소 **≤ 2개**, 각각 `torso`/`compressed`·`torso`/`stretched`, `severity` **0.34**, `permanent` **true**. 같은 방향의 재통과에서 배열이 변하지 않는지 기록한다. **`severity` 합이 `0.34`와 `0.68` 두 값만 나오는지**를 본다. 세 번째 값이 나오면 병합이 깨진 것이다 |

**M3이 이 사양에서 가장 실무적이다.** 통행료는 유일하게 되돌림에 붙는
가격이고, 그 크기가 Kit의 나머지 모든 물리와 어긋나면 스케일 축과 진행 축이
`04` §0.1이 금지한 방식으로 섞인다. **이전 판의 M3은 "합이 기준선 `2.0`을 넘는지"
를 재고 그래서 기준선에 매여 있었다.** 그 질문은 폐기되었다(`04` §4.2.1, `07` Q14
종결). **M3이 재는 질문은 두 개뿐이다.**

1. **유계인가.** `severity` 합이 `0.68`을 넘는 값으로 한 번이라도 나오면 실패다.
2. **종류가 남는가.** 왕복한 몸에 두 `kind`가 다 있는지 확인한다. 하나뿐이면 그 Kit은
   되돌림이 없거나(`04` §5가 정본) 병합이 방향을 잃은 것이다.

**합이 아니라 최댓값이므로 M3은 계측이 아니라 확인이다.** 값이 하나로 고정되어
있으므로 "맞는지"를 재는 문제가 없다. 그것이 이 판이 `0.34`/`0.25` 논쟁을
끝낸 실제 이득이다.
...
| **P6d** | **T2-17, T4-14, T4-15, T4-16, T4-17, T8-18, T8-19, G23, G24 통과** (동일 정체는 `max` 병합, 총량 `≤ 0.68`, 멱등·가환·결합, 정체 중복 0개, authored 병합 지정 0건) | 필수. 깨지면 통행료가 왕복 횟수에 비례해 이 축이 진행 축이 아니라 세션 축이 된다. `04` §4.2.3 |
| **P6e** | **T2-18 통과** (`torso`가 두 `kind`를 함께 갖고 세이브/로드에서 2개로 복원) | 필수. 깨지면 되돌림의 증거가 하나뿐이거나 방향이 증명되지 않는다. `04` §4.8 |
...
| **P8b** | **M3 통과** — `wounds` 원소 ≤ 2개, `severity` 합 ≤ `0.68`, 왕복한 몸에 두 `kind` | 필수. `ROUND_PLAN.md` §11.1d |
...
**P6d가 깨졌을 때의 실패 모드는 그 반대편이다: "되돌릴 수 있는데 값이 자란다."**
통행료가 합산되면 `severity` 총량이 세션 길이의 함수가 되고, 10분 플레이한 저장과
3시간 플레이한 저장이 **같은 축의 다른 상태**가 된다. `0.68`이 이 축의 정의된
끝이므로 그 이상은 저장 계약 위반이다. 자동 테스트가 잡는다(`T4-14`, `T4-17`).
`ROUND_PLAN.md` §11.1d가 이 소유자 결정이고 **상수 조정(`0.34` → `0.25`)은
해결책이 아니다** — 합산 상태에서 어떤 상수도 총량이 무한히 커진다.
...
- Godot 실행 0회. 위의 어떤 테스트도 이 문서를 쓴 시점에 실행되지 않았다

`docs/scale_collapse/**` 안의 8개 markdown 파일과 `docs/world/12_SCALE_RULES.md`가
이 작업의 전체 산출물이다. **통행료 병합 패스는 추가로
`docs/world/13_REFERENCE_EXCLUSIONS.md`의 물 항목 하나를 고쳤다**(그 패스의 소유
범위). 그 밖의 `docs/world/**` 파일은 쓰지 않았다.
