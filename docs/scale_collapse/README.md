...
모든 rung으로 갈 수 있다. 되돌아오는 데 드는 비용은 **통행료** 하나뿐이다: 통과한 rung
하나당 확정 사실(`body.wounds`에 쌓이는 흉터) 하나. 되돌아온 몸은 숫자까지 이전과
정확히 같지만 **몸은 같지 않다.** 그래서 세계관 헌법 불변식 5의 "언제든 불완전하게
돌아온다"가 그대로 성립하고 **예외 규칙이 필요 없다.**
...
않는다. 통행료는 `wounds`에만 쌓이고 `missing`에는 쌓이지 않는다. `wounds`는 rung을
...
| 4 | [`04_TRANSITIONS.md`](./04_TRANSITIONS.md) | **통행료(모든 rung 전환의 유일한 가격)**, 되돌림의 정의와 왕복 산술, 강연결 rung 그래프, PVE 경유 애니메이션 | W2, W4, W5, W6, W7 |
| 5 | [`05_AUTHORED_FORMAT.md`](./05_AUTHORED_FORMAT.md) | 장소/접근로 JSON 스키마, 스케일 변경 트리거 JSON 스키마(**`cost` 키 없음**), 난이도 증가 worked example 4종 | W0, W4, W5, W6 |
| 6 | [`06_TESTS_AND_GATES.md`](./06_TESTS_AND_GATES.md) | 테스트 파일 경로, 각각이 단언하는 것, 정확한 명령, 텍스트 금지 게이트, **되돌림 게이트** | W0, W2, W7, W4, W5, W6 |
| 7 | [`07_OPEN_QUESTIONS.md`](./07_OPEN_QUESTIONS.md) | 확신하지 못한 결정, 권고안, 대안, 승인자. **Q10은 종결**, Q13~Q19가 가역 판에서 추가 | W0, W1, W2, W3, W7 |
...
| 4 | `docs/research/round_2026_09_26/ROUND_PLAN.md` | §7 세 Kit의 서로 다른 축, §8 금지 목록, §11.1 스케일 붕괴 확정 / 물 금지, **§11.1a 6단 사다리 확정**, **§11.1b scale 가역 + 두 부류 확정**, §11.2 레퍼런스 고유 형질 금지, §11.3 플레이어 노출 텍스트 3종 |
...
| **S-INV-5b** | 모든 rung 전환은 **통행료**를 낸다. 통과한 rung 하나당 확정 사실 1개. 통행료가 0인 전환은 `initial` 1개뿐이다. | `ROUND_PLAN.md` §11.1b, `04` §4 | `core/worldstate/tests/test_scale_no_accumulator.gd` + `tests/core/test_scale_rung_graph.gd` |
...
| **R1b** | `core/procedural/scale_toll.gd` (W2) | 통행료 상수 4개. **R1과 별개 파일.** §아래 | `04` §4.1. `Q16` |
...
정하고 **무차원**이라고 명시한다. 통행료는 **방향**(`to < from`), **개수**(`steps`),
**몸**을 안다. `q`는 그 셋을 알 수 없다. `severity: 0.34`은 무차원이 아니다.
**기하는 크기를 모르고 비용은 크기를 안다.** 그래서 `Q16-1`.
...
몸이 그 크기로 왔을 뿐이다.** 되돌아가면 `0.34`이 하나 더 쌓이고 `common`으로
돌아온다.

```
0.65 → 0.28   (1 rung 아래, toll 0.34, compressed)   module_count 6.384 → 2.750
0.28 → 0.65   (1 rung 위,   toll 0.34, stretched)    module_count 2.750 → 6.384
```

`scale`은 정확히 복원된다. `wounds`는 2개 늘어난다. **`03` §6.1`의 하강 도약이
귀가 확인이다.**
...
| `docs/world/12_SCALE_RULES.md` | **내 단독 소유** (W1에게서 인계됨, 2026-09-26) |
...
이 작업이 쓴 파일은 9개다. `docs/scale_collapse/` 안의 `README.md`,
`01_SCALE_ALGEBRA.md`, `02_PLACE_REQUIREMENTS.md`, `03_MISMATCH_VISUALS.md`,
`04_TRANSITIONS.md`, `05_AUTHORED_FORMAT.md`, `06_TESTS_AND_GATES.md`,
`07_OPEN_QUESTIONS.md`, 그리고 `docs/world/12_SCALE_RULES.md`. 다른 파일을 쓰지 않았다.
