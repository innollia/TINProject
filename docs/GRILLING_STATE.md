...
상태: **ACTIVE — project-wide direction grilling**  
갱신: 2026-09-23
...
아래는 **아직 project-wide grilling에서 닫히지 않은 가지**다.

새 채팅은 여기서 시작한다. 위 Settled 항목을 다시 확인하는 질문부터 시작하지 않는다.

## F1 — 장르 전환 시 연속성 계약

A 장르 구간에서 B 장르 구간으로 넘어갈 때 무엇이 **TIN 전체의 연속 상태**이고 무엇이 **Kit-local 상태**인가?
...
- UI/focus
- local fail/death state

기존 구현이 어떤 값을 가지고 있는지는 사실 조사 대상이고, 어떤 의미를 global로 둘지는 사용자 결정이다.

## F2 — 장르 전환의 공간적/서사적 형태

장르가 바뀔 때 항상 하나의 GameModule을 완전히 떠나 다음 GameModule로 가는가, 아니면 “같은 장소/장면이 다른 장르 문법으로 바뀐다”는 경험도 일급 요구사항인가?

기존 ModuleHost 교체 구조를 유지한다는 결정과, 플레이어가 체감하는 전환 연속성을 어떻게 함께 만족시킬지 정해야 한다.

## F3 — Kit가 실제 TIN 본편에 들어가는 방식

Reference Game에서 검증된 Kit를 본편에서 사용할 때:
- Reference Game content만 버리고 system/presentation grammar를 가져오는지
- presentation도 Kit의 기본으로 남기고 TIN 미술만 교체하는지
- 어느 부분까지 Kit 계약이고 어느 부분부터 작품별 자유영역인지

경계를 정해야 계획서가 과도하게 rigid하거나 반대로 무의미하게 느슨해지지 않는다.

## F4 — 사용자 플레이 검토 이후의 수정 루프

“검토 준비 완료”까지는 확정됐다.

아직 정할 것:
- 사용자가 플레이 후 준 피드백을 해당 Kit 계획에 다시 반영한 뒤 재구현할지
- 반복 실패를 어느 수준에서 global rule로 승격할지
- 사용자 승인 전 다음 Kit를 시작할 수 있는지

## F5 — Kit roadmap / 우선순위

project-wide direction이 끝난 뒤:
- 어떤 Kit부터 별도 grilling할지
- 한 번에 하나만 active로 둘지
- parallel Kit 작업을 허용할지
를 정한다.
