...
상단 150px는 전역 UI다. 선택/전환, Save/Load, P/Escape 일시정지, reset 명령, Master 볼륨 저장을 제공한다. 모듈 자식은 process_mode를 상속한다. 정지/전환 동안 모듈 처리와 입력을 차단하되 셸은 살아 있다. 완료 후 자동 이동은 하지 않는다.
