...
- 작업 시작 시 `정체성 검토판`, `합성용 인물`, `배경`, `독립 소품` 중 납품 유형을 명시한다. 실제 게임용 인물 요청을 검토판이나 익명 화풍 실험으로 바꾸지 않는다.
- 합성용 인물은 장면 ID·사건 시점·포즈·카메라·신체 범위·기준 화면에서의 표시 크기·알파·피벗·안전 여백을 적는다. 장면의 가구와 바닥, 배경, 그림자는 인물 이미지에 굽지 않고 별도 배치한다. 손에 든 물건은 인물에 합칠지 독립 자산인지 명시한다.
- 합성용 인물의 납품은 실제 투명 알파가 있는 PNG다. 흰 배경이나 체크무늬를 그린 이미지는 투명 이미지가 아니다. 도구가 알파를 지원하지 않으면 단색 분리용 원본 → 배경 제거 → 알파 확인까지 한 작업으로 명시한다. 중간 불투명 출력은 납품 완료가 아니다.
- A/B마다 `가져올 부분 / 가져오지 않을 부분 / 이번 자산에서 적용하지 않는 부분`을 적고, 실제 첨부 파일 ID와 모델 입력 유형을 실행 기록에 남긴다. `그림체로 사용`이라는 문장만으로 이 계약을 대신하지 않는다.
- 프롬프트의 역할 분리는 모델이 영역별로 적용했다는 증거가 아니다. 모델의 참조 입력 지원과 결과의 화풍은 별도로 확인한다. 일반 IP-Adapter weight를 A/B 역할 마스크처럼 설명하지 않는다.
- 사용자에게 실사·사진풍 실패로 판정된 결과와 실행 설정은 실패 기록에 남기고 다음 입력·Gold Standard에서 제외한다. 사용자 품질 평가를 에이전트 점수로 대체하지 않는다.
- 미확정 장면 정보는 추측하지 않고 해당 항목을 차단 상태로 기록한다. 새 모델 비교도 사용자가 지정한 실제 자산 용도를 유지하고, 스타일 게이트 통과나 게임 사용 승인으로 자동 승격하지 않는다.
...
실행 직전에 압축된 프롬프트와 brief의 필수 상태를 대조한다. 이번 실제 누락 사례인 `바닥에 앉음`처럼 자세의 지지면, 사건 후 붕대, 손에 든 단서, 인물 비례는 압축 과정에서 지우지 않는다. 출력 실패를 분류할 때 입력에서 빠뜨린 요구를 모델의 지시 불이행으로 기록하지 않는다.
...
2026-09-26 사용자 결정: 방향·안전 여백·캔버스 정렬처럼 해결 가능한 항목은 작업자가 먼저 보정한다. 좌우반전은 글자·비대칭 장식·손과 소품의 의미가 바뀌는지 확인하고 적용한다. 원본을 보존하고 파생 파일에 연산을 기록한다. 사소한 보정 가능 항목을 사용자 선택의 차단 사유로 만들지 않는다.

실험 계속이 승인된 경우 결과별 링크를 제공하면서 가설→제한 생성→원본 대비 검토→다음 변경을 수행한다. 명세 및 관찰 가능한 렌더 문법은 작업자가 자체 검토하며 최종 취향/Gold Standard 승격은 사용자가 정한다. 허용된 예산 안에서 진행하며 매 후보마다 승인을 요구하지 않는다. 생성 실패물의 재귀 참조 금지와 구매 확인 원칙은 유지한다.
