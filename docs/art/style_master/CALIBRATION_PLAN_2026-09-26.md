...
- 각 시도는 이전 승인되지 않은 생성 결과를 이미지 입력으로 받지 않는다.
- 현재 사용자가 다시 첨부한 원본 A/B 중 해당 시도에 필요한 이미지만 쓴다.
- 결과가 나오면 먼저 원본 결과를 비교 표에 남긴다.
- 다음 시도는 다시 공통 피사체에서 시작한다.
...
## Test AB — Compact Visual Contract
